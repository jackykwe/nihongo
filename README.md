...
2. Run `./build.sh`. This builds the main PDF (which is being progressively extended). The estimated compilation time is about 5-10min (at time of writing this).
2. Run `./build-supplementary.sh`. This builds the supplementary PDF (contains notes that don't change frequently). The estimated compilation time is about 5min (at time of writing this).
...
To witness how the DOCKER MODE document will look like, without resorting to rendering the big document by executing `build.sh`, execute `build-dev.sh` instead after editing `nihongo-gakushuu-kyouzai.tex` to comment out irrelevant sections. `build-dev.sh` only renders once, so cross-references are all rendered as "??", but the output is sufficient for previewing the final aesthetics of the relevant tables.
