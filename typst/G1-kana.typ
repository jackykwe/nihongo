...
+ (tl;dr: morpheme is repeated) the reading of the repeated 漢字 must be the same, though certain changes are permitted such as _rendaku_ (unvoiced consonant becomes voiced, i.e. the dakuten, e.g. in #ruby[人々][ひと|びと], ひ $\to$ び) and _gemination_ (consonant lengthening, i.e. the っ, e.g. in #ruby[刻々][こっ|こく]), and
...
- Various alterations in meaning
  - #ruby[個][こ] (piece) $\to$ #ruby[個々][こ|こ] (individually)
  - #ruby[時][とき] (time) $\to$ #ruby[時々][とき|どき] (sometimes)
  - #ruby[翌日][よく|じつ] (next day, as in 隔天/隔一天 (CN)) $\to$ #ruby[翌々日][よく|よく|じつ] (next next day, as in 隔两天 (CN))
...
//     Another example:
//     \begin{itemize}
//         \item The different pronuncations #rruby[今日][きょう], #ruby[今日][こん|にち] and #ruby[今日][こん|じつ] are each preferred in different contexts.
//     \end{itemize}
// \end{itemize}
...
// Some special notes:
// \begin{itemize}
//     \item Voiced consonants vibrate the vocal cords, while unvoiced consonants don't (see #link("https://www.tofugu.com/japanese/japanese-pronunciation/\#vocal-cords")[Tofugu] article).
//     \item In the modern 東京 dialect, ず and づ are pronounced exactly the same way: "zu", as expressed in their identical Hepburn romanisation (\S@sec:hepburn-romanisation).
//     \item When in the middle of words, がぎぐげご may be pronounced with a "ng-" start instead of a "g-" start. This is a regional variation (that's not too uncommon); both ways are acceptable (see #link("https://www.tofugu.com/japanese/japanese-pronunciation/\#nasal-")[Tofugu] article).
//     \item The native Japanese speaker will pronounce the "v" family (ヴ、ヴァ、ヴィ、ヴェ、ヴォ、ヴュ) as /b/.
//     \item Vowel extensions (\S@sec:vowel-extension) are pronounced as vowel extensions; do not pronounce the extender if it's a different vowel! For example, #ruby[先生][せん|せい] is pronounced _sen-se_ with an elongated trailing "e" vowel. There is no "i" vowel sound!
//     \item Almost every 漢字 character has two different readings (see \S@sec:on-yomi-and-kun-yomi):
//     \begin{itemize}
...
//         The purpose of trailing 送り仮名 is to preserve the pronunciation of the 漢字, even as the word is conjugated\footnote{*Conjugation*: change of word form to fit a given context.} to different forms. It is also used to differentiate transitive and intransitive verbs (\S@sec:transitive-intransitive-verbs).
//     \end{itemize}
...
//     When repeating 漢字 using 々, _rendaku_ (see \S@sec:rendaku; unvoiced consonant becomes voiced, i.e. the dakuten, e.g. in 人々, ひ $\to$ び) and _gemination_ (consonant lengthening, i.e. the っ, e.g. #ruby[刻々][こっ|こく]) _may_ occur.
// \end{itemize}
...
// {
//     \toprule
//     \SetCell[r=2]{c,m} *Vowel to extend* & \SetCell[c=2]{c,m} *Extend by appending* & & \SetCell[r=2]{c,m} *Example* \\* \cmidrule[lr]{2-3}
//     & *平仮名* & *片仮名* & \\
//     \midrule
...
//     /o/ & \textred{う} \textblue{(お)} & & き\underline{ょう}、おは\underline{よう}、\textblue{(\underline{おお}きい)}、\textblue{(#ruby[遠回][#underline[とお]|まわ]り)} \\
//     \bottomrule
// }
...
// \begin{description}
//     \item[Vowel extension (\S@sec:vowel-extension)] When vowels "a", "e", "o", "u" are extended #underline[as part of the same morpheme], it is expressed with a macron (overbar), and the extender vowel is dropped. \textred{Extension of "i" and the "e+i" combination are exceptions: they remain repeated.}
...
//     \item (#ruby[当][あ]て#ruby[字][じ] I: borrow Chinese reading, #underline[invent] meaning) Part of the precursor to modern-day 平仮名 and 片仮名: #ruby[万葉仮名][ま|にょう|が|な]. This archaic orthographic system used (multiple) Chinese characters to represent each Japanese sound.
