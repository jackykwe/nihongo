...
\begin{enumerate}[label=\arabic*.]
    \item (tl;dr: morpheme is repeated) the reading of the repeated 漢字 must be the same, though certain changes are permitted such as _rendaku_ (unvoiced consonant becomes voiced, i.e.\ the dakuten, e.g.\ in #ruby[人々][ひと|びと], ひ $\to$ び) and _gemination_ (consonant lengthening, i.e.\ the っ, e.g.\ in #ruby[刻々][こっ|こく]), and
...
}  % inner specification options
{
    ...
    \midrule
    ゝ & _ゝ_ & \texttt{onaji} $\to$ space$^\texttt{*}$ & 平仮名 previous character repeater (enforce without dakuten) \\
    ゞ & _ゞ_ & \texttt{onaji} $\to$ space$^\texttt{*}$ & 平仮名 previous character repeater (enforce with dakuten)\\
    ヽ & _ヽ_ & \texttt{onaji} $\to$ space$^\texttt{*}$ & 片仮名 previous character repeater (enforce without dakuten)\\
    ヾ & _ヾ_ & \texttt{onaji} $\to$ space$^\texttt{*}$ & 片仮名 previous character repeater (enforce with dakuten)\\
    々 & _々_ & \texttt{noma} $\to$ space$^\texttt{*}$ & 漢字 previous character repeater (ノ$+$マ) \\
    % & & &  \\
    \bottomrule
}
...
== Pronunciation \label{sec:pronunciation}
...
\begin{itemize}
    ...
    \item Vowel extensions (\S\ref{sec:vowel-extension}) are pronounced as vowel extensions; do not pronounce the extender if it's a different vowel! For example, #ruby[先生][せん|せい] is pronounced _sen-se_ with an elongated trailing ``e'' vowel. There is no ``i'' vowel sound!
    ...
    \begin{itemize}
        ...
        The purpose of trailing 送り仮名 is to preserve the pronunciation of the 漢字, even as the word is conjugated\footnote{*Conjugation*: change of word form to fit a given context.} to different forms. It is also used to differentiate transitive and intransitive verbs (\S\ref{sec:transitive-intransitive-verbs}).
    \end{itemize}
    ...
    When repeating 漢字 using 々, _rendaku_ (see \S\ref{sec:rendaku}; unvoiced consonant becomes voiced, i.e.\ the dakuten, e.g.\ in 人々, ひ $\to$ び) and _gemination_ (consonant lengthening, i.e.\ the っ, e.g.\ #ruby[刻々][こっ|こく]) _may_ occur.
\end{itemize}


=== Vowel extension \label{sec:vowel-extension}
...
== Hepburn romanisation \label{sec:hepburn-romanisation}
...
== 音読み and 訓読み and mixing the two \label{sec:on-yomi-and-kun-yomi}
