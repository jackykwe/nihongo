...
\end{itemize}
Intercardinal and secondary intercardinals use the 音読み readings. The only exception is when cardinals describing direction spans come together, in whcih case \emph{rendaku} occurs, so we have \ruby{東西}{とう|ざい} and \ruby{南北}{なん|ぼく}.
...
}  % inner specification options
{
    ...
    \midrule
    \midrule
    \ruby{弱者}{じゃく|しゃ} & weak personl the weak/vulnerable/disadvantaged & \\
...
}  % inner specification options
{
    ...
    オヤジギャグ & corny joke/dad joke & (\ruby[g]{親父}{おやじ}ギャグ); slang \\
    % & & \\
    \bottomrule
}
